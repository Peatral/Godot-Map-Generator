...
# Taken from https://github.com/hiulit/Delaunator-GDScript/blob/master/Delaunator.gd

const EPSILON = pow(2, -52)
const EDGE_STACK = []

var coords := PackedInt64Array()
var halfedges := PackedInt32Array()
var hull := [] # This array should be a PackedIntArray but we need to use the .slice() function on it.
var triangles := PackedInt32Array()
var triangles_len := 0
var _cx: float
var _cy: float
var _dists := PackedInt64Array()
var _halfedges := [] # This array should be a PackedIntArray but we need to use the .slice() function on it.
var _hash_size: int
...
var _ids := [] # PackedIntArray, but causes errors if not an array
var _triangles := []  # This array should be a PackedIntArray but we need to use the .slice() function on it.
...
#	triangles_len = 0
	...
	# Removed the triangles_len-1 - unnecessary breaking bug
...
func triangle_center(points: PackedVector2Array, t: int) -> Vector2:
	...
	var res = circumcenter(vertices[0].x, vertices[0].y, vertices[1].x, vertices[1].y, vertices[2].x, vertices[2].y);
...
func for_each_voronoi_cell(points: PackedVector2Array, callback: Callable) -> void:
	if index.keys().size() == 0:
		for e in triangles.size():
			var next = next_halfedge(e)
			var endpoint = triangles[next]
